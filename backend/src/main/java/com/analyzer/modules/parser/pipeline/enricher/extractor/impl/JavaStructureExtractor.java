...
public class JavaStructureExtractor implements StructureExtractor {
    ...
    public void extract(CodeChunk chunk) {
        ...
        match(PACKAGE_PATTERN, content).ifPresent(v -> chunk.getMetadata().put("packageName", v));
        match(CLASS_PATTERN, content).ifPresent(v -> chunk.getMetadata().put("className", v));
        match(EXTENDS_PATTERN, content).ifPresent(v -> chunk.getMetadata().put("extends", v.trim()));
        match(IMPLEMENTS_PATTERN, content).ifPresent(v -> chunk.getMetadata().put("implements", v.trim()));
        chunk.getMetadata().put("stereotype", chunk.getChunkType().name().toLowerCase());
    }

    private Optional<String> match(Pattern pattern, String content) {
